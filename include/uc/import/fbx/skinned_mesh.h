...
#include <uc/import/geo/skinned_mesh.h>
#include "fbx_common.h"
#include "fbx_common_multi_material_mesh.h"

#include "fbx_helpers.h"
#include "fbx_transform_helper.h"
#include "fbx_common_transform.h"
        ...
        namespace import
        {
            namespace fbx
            {
                ...
                inline std::vector< std::vector<int32_t> > get_material_indices(const fbxsdk::FbxMesh* mesh)
                {
                    ...
                    if (material_count == 0)
                    {
                        throw_exception<exception>("mesh does not have assigned materials");
                    }
                ...
                inline std::shared_ptr<geo::skinned_mesh> create_skinned_mesh_internal(const fbxsdk::FbxMesh* mesh, const fbx_context* context)
                {
                    ...
                    {
                        throw_exception<exception>("file does not contain a triangle mesh");
                    }
